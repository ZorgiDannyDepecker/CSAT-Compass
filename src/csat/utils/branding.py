...
<style>
    ...
    /* ── Comprimeer gap na sorteerbare-tabel iframes ────────────────────
       Streamlit's stVerticalBlock heeft een vaste gap van 32px tussen
       elk element. Na een iframe-tabel is dat te veel — we willen de
       legenda-tekst direct na de tabel zonder zichtbare tussenruimte.
       De negatieve margin op de VOLGENDE container compenseert de gap. */
...
def inject_iframe_resize() -> None:
    """
    Resize sorteerbare-tabel iframes naar exacte body.scrollHeight.

    Meet bodyScrollH direct — de enige betrouwbare maat ongeacht
    max-height, borders of padding. Wordt uitgevoerd na laden + bij tabwissel.
    """
    import streamlit.components.v1 as _stc_inner
...
(function(){
    ...
    function resizeAll(){
        d.querySelectorAll('iframe').forEach(function(f){
            try{
                ...
                if(!doc) return;
                if(!doc.querySelector('.scroll-wrap')) return;
                var h = doc.body.scrollHeight;
                if(h > 20){
                    f.style.setProperty('height', h+'px', 'important');
                    var c = f.closest('[data-testid="stElementContainer"]');
                    if(c) c.style.setProperty('height', h+'px', 'important');
                }
            }catch(e){}
        });
    }
    ...
    if(!d.__zorgiResizeAttached){
        d.__zorgiResizeAttached = true;
